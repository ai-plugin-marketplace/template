...
/**
 * `dist/` subdirectories owned and regenerated by this builder. They are wiped
 * at the start of every run so the output mirrors `plugins/` exactly — removing
 * or renaming a plugin leaves no stale artifacts behind (no manual cleanup).
 */
export const TARGETS = ["gemini", "kiro"] as const;
...
export function buildAll(root: string = ROOT): void {
  ...
  // Wipe the managed target roots up front so dist/ is a pure function of
  // plugins/ — stale exports from removed/renamed plugins are cleared without
  // any manual `rm`.
  for (const target of TARGETS) {
    cleanDir(path.join(distDir, target));
  }
  ...
  if (plugins.length === 0) {
    console.log("No plugins found in plugins/ — dist/ targets cleaned.");
    return;
  }
  ...
  for (const plugin of plugins) {
    ...
    const geminiDest = path.join(distDir, "gemini", plugin);
    const kiroDest = path.join(distDir, "kiro", plugin);
